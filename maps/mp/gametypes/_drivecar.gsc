...
onDestroyedJeep() 
{
	...
	if(isDefined(self.isarmored))
		self setModel(game["jeepad"]);
	else
...
detectCollition() 
{
	...
	while(true) 
	{
		...
		for( j = -40; j <= 40; j += 8 ) 
		{
			...
			ent = trace["entity"];
			if( isDefined(ent) || (trace["surfacetype"] != "none" && trace["surfacetype"] != "default") ) 
			{
	...
	if( isPlayer( ent ) && isAlive( ent ) && isDefined( ent.pers ) ) 
	{
		...
		if( ent useButtonPressed() == true && !isDefined( ent.jeep ) ) 
		{
			while( ent useButtonPressed() == true ) 
			{
				if( isDefined( self.driver ) ) 
				{
					if( isDefined( self.driver.pers ) && self.driver.pers["team"] != ent.pers["team"] )
						return;
				}

				if( isDefined( self.passenger1 ) ) 
				{
					if( isDefined( self.passenger1.pers ) && self.passenger1.pers["team"] != ent.pers["team"] )
						return;
				}
...
switchPlaces() 
{
	...
	if(!isDefined(jeep.isarmored))
		...
		self iprintlnBold( "Press ^2Z ^7to shoot!" );
	...
	{
		self setClientCvar ("clientcmd", "bind z openscriptmenu -1 z");
		self openMenu ("clientcmd");
		self closeMenu ("clientcmd");
		...
		while( isDefined( self ) && isAlive( self ) && isDefined( jeep )) 
		{
			...
			{
				iFireMG = 0;
				tempMG = vector_scale(anglesToForward( jeep.angles ), 56);
				tempMG += jeep getOrigin() + (0,0,78);

				thread maps\mp\gametypes\_admin::PlaySoundAtLocation("tank_buffalo_fire", tempMG + (0,0,10), 7);

				jeep thread mg_bullet(tempMG, anglesToForward( jeep.angles),self,1.5,17580, 0.3,"jeep_mp");
				playFX( level._effect["tank_mg_flash"], tempMG, anglesToForward( jeep.angles ) );
			}
			else
				iFireMG = 1;
...
mg_bullet(origin,direction,peep,iTime,ivel,idamage,weapon)
{
	self endon("intermission");
	
	temp = vector_scale(direction,100);
	origin += temp;
	waitTime = 1;
	pos = origin;
	impactTime = ( 1 / ivel );
	...
	while( iTime > 0 )
	{
		temp = vector_scale( direction,1500 );	
		trace = bulletTrace( origin, origin + temp, true, undefined );
		...
		{		
			...
			{	
				ent thread [[level.callbackPlayerDamage]](peep, peep, 100, 1, "MOD_PISTOL_BULLET", weapon, pos, direction, "none",0);
...
detachPlayer() 
{
	...
	while(isDefined( self.jeep ) && isAlive( self )) 
	{
		...
		if( self attackButtonPressed() == true && self meleeButtonPressed() == false ) 
		{
			...
			{
				if( !isDefined( jeep.passenger1 ) && !isDefined(jeep.isarmored) ) 
				{
					jeep.driver = undefined;
					jeep.passenger1 = self;
...
spawn_jeep(origin, armor) 
{
	jeep = spawn("script_model", origin );

	if(isDefined(armor) && armor == "1")
		jeep.isarmored = true;
		
	if(isDefined(jeep.isarmored))
		jeep setModel( game["jeepa"] );
	else
...
precacheStuff() 
{
	...
	game["jeepa"] = "xmodel/vehicle_german_armored_car";
	game["jeepad"] = "xmodel/vehicle_german_armored_car_d";

	precachemodel( game["jeepa"] );
	precachemodel( game["jeepad"] );
	...
	level._effect["tank_mg_flash"] = loadfx("fx/muzzleflashes/armored_car_20mm.efx");
}