...
build()
{
	trig = getent("buildaa", "targetname");

	level.flangunisactive = false;
	
	while(1)
	{
		trig waittill("trigger", user);

		if(!level.flangunisactive)
		{
			completed = user ProgressBar(trig);

			if(isDefined(completed))
			{
				user iprintlnbold("^1You planted a flangun^4!");
				iprintln(user.name + " ^1Has Planted A Flangun!");
				thread aa_spawn();
				trig maps\mp\_utility::triggerOff();
				level.flangunisactive = true;
			}
		}
	}
}
...
aa_spawn()
{
	...
	spawn = getent("aaspawn", "targetname");	
	
	trace = bulletTrace( spawn.origin, spawn.origin - (0,0,500), false, self );

	aa = spawn("script_model", trace["position"]);
	aa setModel( game["aa"] );
	aa show();
	aa.angles = (25, -90, 0);
