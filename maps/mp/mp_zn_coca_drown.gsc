...
main()
{
	...
	drownage = getentarray("trig_water","targetname");
...
drown(trigger)
{
	...
	while (isAlive(self) && self istouching(trigger) && !self istouching(dceiling))
	{
		...
		while(isAlive(self) && self istouching(trigger) && !self istouching(dceiling) && progresstime < level.drowntime)
		{		
			f++;
		...
		if(isAlive(self) && self istouching(trigger) && !self istouching(dceiling) && progresstime >= level.drowntime)
		{
